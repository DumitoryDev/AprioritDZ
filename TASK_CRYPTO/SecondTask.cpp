#include <openssl/dh.h>
#include <openssl/evp.h>
#include <openssl/err.h>
#include <iostream>
#include <iomanip>
#include <sstream>
#include <functional>

//openssl 1.0.2.6

const char* userA_PrivateKey = "";

const char* userA_PublicKey = "";

const char* userB_PublicKey = "";

const char* p = "";
const char* g = "2";


using u_ssl_ptr =  const std::unique_ptr<void, std::function<void(void* ptr)>>;

auto get_general_secret(
	const std::string& a_private,
	const std::string& a_public,
	const std::string& b_public,
	const std::string& g,
	const std::string& p
)
{
	auto dh = DH_new();
	
	if (dh == nullptr)
	{
		throw std::runtime_error(
			"Error DH_new! code - "
			+ std::to_string(ERR_get_error()));

	}

	u_ssl_ptr u_ptr_dh(dh, [](void* ptr)
		{
			DH_free(static_cast<DH*>(ptr));
		});

	BN_dec2bn(&dh->g, g.c_str());
	BN_hex2bn(&dh->p, p.c_str());
	BN_hex2bn(&dh->priv_key, a_private.c_str());
	BN_hex2bn(&dh->pub_key, a_public.c_str());

	BIGNUM* pub_key_b{};
	BN_hex2bn(&pub_key_b, b_public.c_str());
	
	auto* secret = OPENSSL_malloc(sizeof(unsigned char) * DH_size(dh));
	if (secret == nullptr)
	{
		throw std::bad_alloc();
	}

	u_ssl_ptr u_ptr_secret(
		secret,
		[](void* ptr)
		{

			OPENSSL_free(ptr);

		});

	auto size{ 0 };

	if ((size = DH_compute_key(static_cast<unsigned char*>(secret), pub_key_b, dh)) == -1)
	{
		throw std::runtime_error("Error compute secret!");
	}

	std::string secret_hex(size, '\0');
	std::ostringstream os;

	for (auto i{ 0 }; i < size; ++i)
	{
		os << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(static_cast<unsigned char*>(secret)[i]);
	}

	return os.str();
}

int main(void)
{
	try
	{
		std::cout << get_general_secret(
			userA_PrivateKey, 
			userA_PublicKey, 
			userB_PublicKey, 
			g, 
			p);
		/*
		 * 
		 */
		return 0;
	}
	catch (...)
	{
	}

}